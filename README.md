...
- **v5.4** — a doctor in the box, and batteries that belong to their device. `linkd doctor` walks the whole setup — service, binaries, Bluetooth beacon, per-desktop second-screen dependencies, uinput, audio player, firewall — and prints a checkmark or a fix for each line; its output is the thing to share when asking for help. And with several devices connected, each row in Settings now shows its own battery instead of whichever phone reported last.
...
- **v5.1.1** — windows that belong to the desktop. On Wayland the PC windows stop wearing winit's fallback titlebar: they draw their own chrome — one rounded surface edge to edge, a drag-anywhere header, monochrome window buttons, double-click to maximise — in the same design system as everything inside them.
- **v5.1** — messages you can actually read and hear. Clicking a mirrored notification on the PC opens the full message in its own window — the complete conversation for messaging apps, not the one truncated line — with a copy button. And a new "Phone audio on PC" switch streams the phone's media sound to the computer's speakers, so voice messages play where you are working. Android asks once per session for the capture permission; apps that forbid audio capture stay silent by their own choice.
